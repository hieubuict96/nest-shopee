...
export function sendPhoneNumberAction(Phone, setStep, setError) {
  return async (dispatch) => {
    try {
      ...
      const response = await axios.post("/api/user/signup/send_phone_number", {
        phoneNumber,
      });
      setStep(2);
      setError("");
...
export const sendCodeAction = async (Phone, code, setStep, setError) => {
  try {
    if (!code) {
    ...
    const phoneNumber = `+84${phone}`;
    const response = await axios.post("/api/user/signup/send_code", {
      phoneNumber,
      code,
    });
...
export const sendInfoAction =
  (Phone, firstName, lastName, password, repassword, setError) =>
  async (dispatch) => {
    try {
      if (!firstName) {
...
export const signinGoogleAction = (data) => async (dispatch) => {
  try {
    const response = await axios.post("/api/user/signin_with_google", { data });
    const { user, accessToken } = response.data;
...
export const signinFacebookAction = (data) => async (dispatch) => {
  try {
    const response = await axios.post("/api/user/signin_with_facebook", {
      data,
    });
...
export function sendCodeToEmailAction(
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        "/api/user/profile/update/email/send_code",
        {
          _id,
          newEmail,
          password,
...
export const verifyCodeAction =
  ...
  async (dispatch) => {
    try {
      ...
      const response = await axiosInstance.post(
        "/api/user/profile/update/email/verify_code",
